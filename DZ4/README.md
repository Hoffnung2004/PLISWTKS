...
## Основная часть
### Выбор варианта
По требованию 5 варианта следует сделать ФВЧ фильтр с порядком 35, начало полосы пропускания - 10 МГц, частота дескретизации - 180 Мгц.
### Синтез фильтра
...
Ниже приведены, полученные коэффициенты с точностью до 4-го знака после запятой.
```
h = [  
...
Фактически это очень ограничивает параметры в срипте. Больше порядок фильтра - больше разрядность, а тут уже и так значение, близкое к пределу.
